...
class CreateEmployeeRequest(BaseModel):
    username: str
    password: str
    role: UserRole = UserRole.notenant  # Por defecto es empleado
    email: str | None = None # Empleados empiezan sin correo
...
async def create_employee(
    ...
    create_user_model = Users(
        ...
        email=create_employee_request.email, # Si no se envía, será None (Null en BD)
        role=create_employee_request.role, 
        owner_id=user['id']
    )
    db.add(create_user_model)
    db.commit()
...
async def get_my_employees(user: user_dependency, db: db_dependency):
    ...
    for person in staff_list:
        ...
        if person.role == UserRole.notenant:
