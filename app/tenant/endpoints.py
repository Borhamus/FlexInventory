...
from app.tenant.dependencies import get_tenant_db  # NUEVO
...
def create_inventario(
    ...
    """Crear un nuevo inventario en el tenant"""
    db_inventario = db.query(models.Inventario).filter(
        models.Inventario.nombre == inventario.nombre
    ).first()
...
def create_item(
    ...
    """Crear un nuevo item en el tenant"""
    # Verificar que el inventario existe
    inventario = db.query(models.Inventario).filter(
        models.Inventario.id == item.inventario_id
    ).first()
...
def update_item(
    ...
    if 'inventario_id' in update_data:
        inventario = db.query(models.Inventario).filter(
            models.Inventario.id == update_data['inventario_id']
        ).first()
        if not inventario:
            raise HTTPException(status_code=404, detail="Inventario no encontrado")
...
def create_catalogo(
    ...
    db_catalogo = db.query(models.Catalogo).filter(
        models.Catalogo.nombre == catalogo.nombre
    ).first()
...
def get_catalogos(
    ...
    """Obtener todos los catálogos del tenant"""
...
def get_catalogo(
    ...
    """Obtener un catálogo con sus items"""
